...
namespace Estranged.Automation.Runner.Discord.Responders
{
    public class NaturalLanguageResponder : IResponder
    {
        ...
        public async Task ProcessClassification(IMessage message, CancellationToken token)
        {
            ...
            {
                var classification = await _languageServiceClient.ClassifyTextAsync(Document.FromPlainText(message.Content.Substring(command.Length)), token);
        ...
        public async Task ProcessSentiment(IMessage message, CancellationToken token)
        {
            ...
            using (message.Channel.EnterTypingState())
            {
                var sentiment = await _languageServiceClient.AnalyzeSentimentAsync(Document.FromPlainText(message.Content.Substring(command.Length)), token);
